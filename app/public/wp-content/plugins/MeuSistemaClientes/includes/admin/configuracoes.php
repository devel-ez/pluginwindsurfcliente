        ...
        if (check_admin_referer('msc_configuracoes')) {
            ...
            if (!empty($_FILES['assinatura_responsavel']['tmp_name'])) {
                ...
                if (!isset($upload['error'])) {
                    // Converter URL para caminho absoluto do servidor
                    $upload_dir = wp_upload_dir();
                    $file_path = str_replace($upload_dir['baseurl'], $upload_dir['basedir'], $upload['url']);
                    
                    // Salvar o caminho do arquivo ao invés da URL
                    update_option('msc_assinatura_responsavel', $file_path);
